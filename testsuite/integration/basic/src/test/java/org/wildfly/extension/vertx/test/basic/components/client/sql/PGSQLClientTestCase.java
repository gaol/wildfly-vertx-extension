...
@RunAsClient
public class PGSQLClientTestCase extends ContainerBasedTestCase {

  @ArquillianResource
  private URL url;

  @ClassRule
  public static ContainerBaseRule rule = new ContainerBaseRule("postgres:10.10", 5432) {
    @Override
    protected void updateContainerBeforeStart(final GenericContainer<?> container) {
      final MountableFile mountableFile = MountableFile.forClasspathResource("init-postgres.sql");
      container
        .withEnv("POSTGRES_USER", "postgres")
        .withEnv("POSTGRES_PASSWORD", "postgres")
        .withEnv("POSTGRES_DB", "geo")
        .withFileSystemBind(mountableFile.getFilesystemPath(), "/docker-entrypoint-initdb.d/init-postgres.sql", BindMode.READ_ONLY)
      ;
    }
  };
  ...
  public static Archive<?> deployment() {
    return ShrinkWrap.create(WebArchive.class, "test-pgsql.war")
      .addAsWebInfResource(EmptyAsset.INSTANCE, "beans.xml")
      .addClasses(AbstractAsyncServlet.class, ContainerBasedTestCase.class, PGSQLClientTestCase.class, PGSQLClientServlet.class);
  }

  @Override
  protected String getContainerConnStr() {
    return "postgres://postgres:postgres@" + rule.getContainerIPAddress() + ":" + rule.getMappedPort() + "/geo";
  }

  @Test
  public void testMySQLQuery() throws Exception {
    String res = HttpRequest.get( url.toExternalForm() + "test-pgsql?connStr=" + getServiceConnStrEncoded(), 10, TimeUnit.SECONDS);
